...
Replace Sandcastle's Phase 3 (direct merge to master) with a stacked pull request workflow. Each issue gets its own PR; dependent issues target the previous PR's branch, forming a chain. This gives humans a review gate before any code lands on master, and keeps the git history linear and easy to bisect.
...
- Every change gets a review gate (PR approval) before landing on Replace Sandcastle's Phase 3 (direct merge to master) with a stacked pull request workflow. Each issue gets its own PR; dependent issues target the previous PR's branch, forming a chain. This gives humans a review gate before any code lands on master, and keeps the git history linear and easy to bisect.
